...
パスワードそのものを格納するのではなくハッシュ値(入力から一位に定まる不可逆的に圧縮した値)を格納するとか、
...
このように、本人であることの根拠の提示してくれるベンダ, サービス, 主体のことをIdP(_Identity Provider_)と呼びます。
...
**クライアント(ブラウザ、スマホアプリ)が持っているデータは、すべて攻撃者に読まれたり改ざんされたりする可能性がある**、ということです。
...
現実の鍵では閉める鍵と開ける鍵は同じなので(つまり共通鍵)なので、そんなことできるの?という気持ちになりますが、
...
Phase 1 で見た `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...` という長い文字列は、JWT です。
...
なお、IdPからのトークン発行に関して標準化されたプロトコルとしてOAuth2.0やOpenIDConnectというものが存在するので、
